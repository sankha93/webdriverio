...
function RequestHandler(options, eventHandler, logger) {
    ...
    this.pendingData = [];
    this.pendingFullRequestOptions = [];

    this.socket = io.connect(this.defaultOptions.protocol + '://' +
    ...
    this.socket.on('response', function(body) {
        var defer = this.pendingDefers.shift();
        var requestOptions = this.pendingRequestOptions.shift();
        var data = this.pendingData.shift();
        var fullRequestOptions = this.pendingRequestOptions.shift();
...
RequestHandler.prototype.create = function(requestOptions, data) {
    ...
    /*if (!this.socketConnected) {
        var error = new ErrorHandler.RuntimeError({
            status: -1,
            type: 'ECONNREFUSED',
            message: 'Couldn\'t connect to selenium server',
            orgStatusMessage: 'Couldn\'t connect to selenium server'
    ...
    this.socket.emit(fullRequestOptions.method || 'GET', {
        path: fullRequestOptions.uri.path,
        body: data
    });

    this.pendingDefers.push(defer);
    this.pendingRequestOptions.push(requestOptions);
    this.pendingData.push(data);
    this.pendingFullRequestOptions.push(fullRequestOptions);
